Ext.define('CodenTonic.tools.PolygonQueryButton', {
    ...
    updateStatusText: '',
    renderConfirmMsg: '',
    iconCls: 'icon-polygon-query',
    tooltip: 'Ευρέση Στοιχείων Ορθών Γεωργικών Πρακτικών',
    requires: [
        'GeoExt.Action'],
    ...
    requestFeatures: function(filter) {
        ...
        if (queryLayers.length > 0 ) {
            ...
            me.lastRequest = Ext.Ajax.request({
                ...
                params: {
                    filter: filter,
                    typeName: 'ELPHO:ENV_AREAS,ELPHO:GAEC_VLI,ELPHO:GAEC_VPG',
                    ...
                    if (text !== "") {
                        ...
                        if(result.features.length > 0) {
                            ...
                            for(var i = 0; i < result.features.length; i++)
                            {
                                ...
                                if(result.features[i].id.substring(0, 'ENV_AREAS'.length) === 'ENV_AREAS')
                                {
                                    if(titleA == false){
                                    msg +="<b>ΠΕΡΙΒΑΝΤΟΛΟΓΙΚΕΣ ΠΕΡΙΟΧΕΣ</b>: <br>";
                                        titleA = true;
                                    }
                                    msg += item.properties.ENV_NAME +" " + item.properties.ENV_TYPE+"<br>"
                                }
                                if(result.features[i].id.substring(0, 'GAEC_VPG'.length) === 'GAEC_VPG')
                                {
                                    if(titleB == false){
                                    msg +="<b>ΠΟΛΥΓΩΝΙΚΕΣ ΠΕΡΙΟΧΕΣ ΟΡΘΩΝ ΓΕΩΡΓΙΚΩΝ ΠΡΑΚΤΙΚΩΝ</b>: <br>";
                                        titleB = true;
                                    }
                                    msg += getGaectype(item.properties.GAEC_TYPE_ID)+"<br>"
                                }
                                if(result.features[i].id.substring(0, 'GAEC_VLI'.length) === 'GAEC_VLI')
                                {
                                    if(titleC == false){
                                    msg +="<b>ΓΡΑΜΜΙΚΕΣ ΠΕΡΙΟΧΕΣ ΟΡΘΩΝ ΓΕΩΡΓΙΚΩΝ ΠΡΑΚΤΙΚΩΝ</b>: <br>";
                                        titleC = true;
                                    }
                                    msg += getGaectype(item.properties.GAEC_TYPE_ID)+"<br>"
                                }
                        ...
                        {
                            CustomMessage("Στοιχεία Περιοχής", "Η Επιλεγμένη Περιοχή δεν διαθέτει στοιχεία ορθών γεωργικών" +
                                " πρακτικών καθώς δεν ανοίκει σε περιοχές περιβαντολογικής προστασίας");
                        }

                        CustomMessage("Στοιχεία Περιοχής", msg);
