<html>
<head>
<meta charset="utf-8">
<title>GeoExt 2 Generic GIS WEB APP</title>

    <!--LANGUAGE SELECTION JS FILES-->
    <script async=false defer=false src="../resources/js/libs/languages/language-selector.js"></script>
    <!--EXTJS  JS/CSS FILES-->
    <link href="http://cdn.sencha.io/ext/gpl/4.2.1/resources/css/ext-all-gray.css" rel="stylesheet" />
    <script src="http://cdn.sencha.com/ext/gpl/4.2.1/ext-all.js"></script>
    <link rel="stylesheet" type="text/css" href="../resources/css/example.css"/>
    <!--CUSTOM APPLICATION ICONS-->
    <link href="../resources/css/icon.css" rel="stylesheet" />
    <!--THE WORLD FAMOUS OPENLAYERS-->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/openlayers/2.13.1/OpenLayers.debug.js"></script>
    <!--THE WORLD FAMOUS PROJ4JS SCRIPT-->
    <script src="../resources/js/thirdParty/proj4js-combined.js"></script>
    <!--CONFIG SCRIPT-->
    <script src="../resources/js/libs/Config.js"></script>
    <!--LOADER SCRIPT-->
    <script src="../resources/js/LoaderScript.js"></script>
    <!--MAP RELATED HELPER SCRIPTS-->
    <script src="../resources/js/libs/GISHelper.js"></script>
    <!--GENERIC HELPER SCRIPTS-->
    <script src="../resources/js/libs/Helper.js"></script>
    <!--PRINTING TOOLS-->
    <script src="../resources/js/libs/MapReport.js"></script>
    <!--IDENTIFY BUTTONS-->
    <script src="../resources/js/libs/Identify.js"></script>
    <!--DOWNLOAD AREAS-->
    <script src="../resources/js/libs/SaveFeatureArea.js"></script>
    <!--MEASURE TOOLS-->
    <script src="../resources/js/libs/Measure.js"></script>
    <!--EDIT TOOLS-->
    <script src="../resources/js/libs/edit/DrawFeature.js"></script>
    <script src="../resources/js/libs/edit/SaveChanges.js"></script>
    <script src="../resources/js/libs/edit/SelectFeature.js"></script>
    <script src="../resources/js/libs/edit/SelectPolygonFromWms.js"></script>
    <script src="../resources/js/libs/edit/SimplifyPolygon.js"></script>
    <!--LOGIN FORM-->
    <script src="../resources/js/libs/LoginForm.js"></script>
    <!--OPTION FORM-->
    <script src="../resources/js/libs/Options.js"></script>
    <!--CODE FOR PANELS-->
    <script src="../resources/js/panels.js"></script>
    <!--MAIN APPLICATION CODE-->
    <script src="../resources/js/geoapp.js"></script>

</head>
<body> </body>
</html>
